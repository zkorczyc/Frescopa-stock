...
export function createFrescopaServer(supabase: SupabaseClient): McpServer {
  ...
  server.tool(
    "frescopa_list_stores",
    "List Fréscopa stores (~100 globally). Filter by region (americas|europe|asia), country, or city.",
    {
  ...
  server.tool(
    ...
    "Per-store stock. Filter by city, store code (e.g. EU-012, AM-001), region, or country. At least one filter required.",
    {
      city: z.string().optional(),
    ...
    async ({ city, store_code, region, country }) => {
      ...
      if (region) storeQuery = storeQuery.eq("region", region);
      if (country?.trim()) storeQuery = storeQuery.ilike("country", country.trim());

      const { data: stores, error: se } = await storeQuery;
      if (se) throw new Error(se.message);
      if (!stores?.length) return jsonText({ stores: [], inventory_lines: [], note: "No stores match the filter." });

      const storeIds = stores.map((s) => s.id);
      const { data: rows, error: ie } = await supabase
        .from("inventory")
        .select(
          "qty_on_hand, updated_at, product:products(demo_product_id,sku,name,category,unit,base_price_cents,currency,image_url), store:store_locations(code,city,name,region,country)"
        )
  ...
  server.tool(
    ...
    "Search product by name/sku/demo_product_id. Returns stock per store; use region to limit rows (default all ~100 stores).",
    {
    ...
    async ({ query: qstr, region, in_stock_only }) => {
      ...
      let invQuery = supabase
        ...
        .in("product_id", ids);
      if (in_stock_only) invQuery = invQuery.gt("qty_on_hand", 0);
      const { data: inv, error: ie } = await invQuery;
      if (ie) throw new Error(ie.message);

      const filtered = region
        ? (inv ?? []).filter((row) => {
            const store = row.store as { region?: string } | null;
  ...
  server.tool(
    "frescopa_regional_availability",
    "Analytics: per region (and country) how many stores stock a product, total qty, availability %. Requires migration v_inventory_by_region.",
    {
    ...
    async ({ query: qstr, region }) => {
      ...
      const base = () =>
        supabase.from("products").select("demo_product_id, name, category").eq("is_active", true);
      ...
      const products = [...map.values()];
      if (!products.length) return jsonText({ note: "No product match." });
  ...
  server.tool(
    ...
    "Promotion rules valid today (codes, % off, category or single-SKU scope).",
    {},
    async () => {
  ...
  server.tool(
    "frescopa_products_on_promotion",
    "Products on promotion TODAY with sale price (promotional_price_cents). Uses view v_products_on_promotion — includes category-wide and SKU-specific deals.",
    {
    ...
    async ({ category, promo_code, scope }) => {
      ...
      if (category) q = q.eq("category", category);
      if (promo_code?.trim()) q = q.eq("promo_code", promo_code.trim());
      if (scope) q = q.eq("promotion_scope", scope);
      const { data, error } = await q;
      if (error) throw new Error(error.message);
      return jsonText({
        as_of: today,
  ...
  server.tool(
    "frescopa_promotion_summary",
    "Count of SKUs covered per active promo code (category promos = many products).",
    {},
    async () => {
      const today = new Date().toISOString().slice(0, 10);
      const { data, error } = await supabase
